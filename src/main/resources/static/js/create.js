    ...
    async function addCriteriaToList(id) {
        ...
        if (!criteriaWrap) {
            ...
            newCriteria.innerHTML =
                ...
                '<td>' +
                (criteria.section === ''
                    ? '<input type="number" name="criteriaMaxValueDtoList[' + id + '].maxValue"  class="form-control form-control-sm w-50" required min="1" value="1">'
                    : criteria.coefficient + '<input type="hidden" name="criteriaMaxValueDtoList[' + id + '].maxValue" value="1" >'
                ) +
                '</td>' +
                '<td>' +
            ...
            if (!resp.ok) {
                ...
                newCriteria.innerHTML =
                    ...
                    (data.section === ''
                        ...
                        : data.coefficient + '<input type="hidden" name="criteriaMaxValueDtoList[' + createdId + '].maxValue" value="1"/> ')+
                    '</td>' +
                    '<td>' +
...
function putIndexesToListAndValidate(event){
    let criteriaList = document.querySelector('.criterion-list');
