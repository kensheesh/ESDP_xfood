    ...
    if (criterion && criterion.length > 0) {
        ...
        for (let i = 0; i < criterion.length; i++) {
            ...
            newCriteria.innerHTML =
                ...
                '<td>' +
                    '<input type="number" name="criteriaMaxValueDtoList[' + criterion[i].id + '].maxValue"  class="form-control form-control-sm w-75" required  min="1" value="'+criterion[i].maxValueType+'" id="maxValueInput-' +criterion[i].id + '">'+
                '</td>' +
                '<td>' +
